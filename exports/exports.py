from src.domain.use_cases.doctor_chat_service import DoctorChatService
...
class DIContainer:
    ...
    def get_chat_service(self):
        if "chat_service" not in self.__instances:
            self.__instances["chat_service"] = ChatService(
                self.get_chat_response_repository(), self.get_database_repository()
            )
    ...
    def get_user_service(self):
        if "user_service" not in self.__instances:
            self.__instances["user_service"] = UserService(
                self.get_database_repository(), self.get_summaary_repository()
            )
        return self.__instances["user_service"]

    def get_doctor_chat_service(self):
        if "doct_chat_service" not in self.__instances:
            self.__instances["doctor_chat_service"] = DoctorChatService(
                self.get_chat_response_repository()
            )
        return self.__instances["doctor_chat_service"]
...
def get_doctor_chat_service():
    return container.get_doctor_chat_service()